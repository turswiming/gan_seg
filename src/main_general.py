...
def create_dataloaders_general(config):
    ...
    if config.dataset.val_name == "AV2_SceneFlowZoo_val":
        ...
        val_dataset = Argoverse2CausalSceneFlow(
            root_dir=Path(config.dataset.AV2_SceneFlowZoo_val.root_dir),
            with_ground=config.dataset.AV2_SceneFlowZoo_val.with_ground,
            use_gt_flow=config.dataset.AV2_SceneFlowZoo_val.use_gt_flow,
            eval_type=config.dataset.AV2_SceneFlowZoo_val.eval_type,
            expected_camera_shape=config.dataset.AV2_SceneFlowZoo_val.expected_camera_shape,
            eval_args=dict(output_path=config.dataset.AV2_SceneFlowZoo_val.eval_args_output_path),
            with_rgb=config.dataset.AV2_SceneFlowZoo_val.with_rgb,
            flow_data_path=Path(config.dataset.AV2_SceneFlowZoo_val.flow_data_path),
            range_crop_type="ego",
        )
    ...
    pass
def forward_scene_flow_general(sample_firsts, sample_nexts, scene_flow_predictor):
    device = next(scene_flow_predictor.parameters()).device
    visible_valid_mask_first = [item.pc.mask & item.flow.mask for item in sample_firsts]
    visible_valid_mask_next = [item.pc.mask & item.flow.mask for item in sample_nexts]
    pc0 = [item.pc.full_global_pc.points[mask,:] for item, mask in zip(sample_firsts, visible_valid_mask_first)]
    pc1 = [item.pc.full_global_pc.points[mask,:] for item, mask in zip(sample_nexts, visible_valid_mask_next)]
...
def forward_mask_prediction_general(sample_firsts, mask_predictor):
    device = next(mask_predictor.parameters()).device
    visible_valid_mask_first = [item.pc.mask & item.flow.mask for item in sample_firsts]
    pc0 = [item.pc.global_pc.points[mask,:] for item, mask in zip(sample_firsts, visible_valid_mask_first)]

    data = {'points': full_pc0s}
...
def main(config, writer):
    ...
    # Create dataloaders
    dataloader, val_dataloader = create_dataloaders_general(config)
    # step = 0
    ...
    with tqdm(dataloader, desc="Training", total=config.training.max_iter-step) as dataloader:
        ...
        for sample in dataloader:
            ...
            visible_valid_mask_first = [(item.pc.mask & item.flow.mask).reshape(-1) for item in sample_firsts]
            print("visible_valid_mask_first shape", visible_valid_mask_first[0].shape)
            print("point_cloud_first shape", sample_firsts[0].pc.full_global_pc.points.shape)
            point_cloud_firsts = [item.pc.full_global_pc.points[mask,:] for item, mask in zip(sample_firsts, visible_valid_mask_first)]
            visible_valid_mask_next = [(item.pc.mask & item.flow.mask).reshape(-1) for item in sample_nexts]
            point_cloud_nexts = [item.pc.full_global_pc.points[mask,:] for item, mask in zip(sample_nexts, visible_valid_mask_next)]
            ...
            print("point_cloud_firsts shape", point_cloud_firsts[0].shape)
            ...
            pred_flow_object = forward_scene_flow_general(sample_firsts, sample_nexts, scene_flow_predictor)
            pred_flow = [item.ego_flows.squeeze(0) for item in pred_flow_object]
            # Forward pass for mask prediction
            pred_mask = forward_mask_prediction_general(sample_firsts, mask_predictor)
            ...
            # Handle evaluation
            handle_evaluation_general(config, step, scene_flow_predictor, mask_predictor, dataloader, device, writer)
