...
class AV2SceneFlowZoo(Argoverse2CausalSceneFlow):
    ...
    def transform_pc(
        self, pc: "torch.Tensor", transform: "torch.Tensor"
    ) -> "torch.Tensor":
        """
    ...
    def __getitem__(self, index):
        ...
        tile_mask_first = half_mask_first_1 & half_mask_first_2
        tile_mask_next = half_mask_next_1 & half_mask_next_2
        point_cloud_first = point_cloud_first[tile_mask_first, :]
        point_cloud_next = point_cloud_next[tile_mask_next, :]

        point_cloud_first = torch.from_numpy(point_cloud_first).float()
        point_cloud_next = torch.from_numpy(point_cloud_next).float()

        # process flow
        flow = sample[0].flow.full_flow[tile_mask_first, :]
        ...
        class_ids = class_ids[tile_mask_first,]
        valid_mask_first = valid_mask_first[tile_mask_first,]
        valid_mask_next = valid_mask_next[tile_mask_next,]
        if hasattr(sample[0], "instance_ids"):
            mask = sample[0].instance_ids[tile_mask_first]
        ...
        if mask is not None:
            ...
            mask_onehot = mask_onehot[
                :, mask_onehot.sum(dim=0) > self.min_instance_size
            ]
        ...
        new_sample = {
            ...
            "valid_mask_first": valid_mask_first,
            "valid_mask_next": valid_mask_next,
        }
        if mask is not None:
    ...
    def downsample_point_cloud(
        ...
        random_indices_first = torch.randint(
            0, point_cloud_first.shape[0], (size,), generator=generator
        )
        ...
        random_indices_next = torch.randint(
            0, point_cloud_next.shape[0], (size,), generator=generator
        )
