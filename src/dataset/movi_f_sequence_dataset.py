...
class MOVIFSequenceDataset(nn.Module):
    ...
    def _load_sequence_data(self):
        ...
        depth_files = sorted(
            [f for f in os.listdir(self.dataset_path) if f.startswith("depth_") and f.endswith(".tiff")]
        )
        seg_files = sorted(
            [f for f in os.listdir(self.dataset_path) if f.startswith("segmentation_") and f.endswith(".png")]
        )
    ...
    def prepare_item(self, idx, from_manual=False):
        ...
        origin_flow = point_cloud_second - point_cloud_first
        coord = "World"
        if coord == "Camera":
            import math
            rotation_angle = 0.03
            ...
            point_cloud_second = point_cloud_second @ rotation_matrix
        if coord == "Projection":
            import math
            rotation_angle = 0.03
            rotation_matrix = torch.tensor(
            ...
            point_cloud_second = point_cloud_second @ rotation_matrix
            K = [[1.09375, 0.0, -0.5], [0.0, -1.09375, -0.5], [0.0, 0.0, -1.0]]
            K = torch.tensor(K).to(torch.float32)
            point_cloud_second = point_cloud_second @ K
            point_cloud_first = point_cloud_first @ K
        # Calculate flow (movement between frames)
        flow = point_cloud_second - point_cloud_first

        # Calculate motion mask
        motion_mask = torch.norm(origin_flow, dim=1) > self.motion_threshold
        ...
        sample = {
            ...
            "point_cloud_next": point_cloud_second,
            "flow": flow,
            ...
            "self": self,
            "valid_mask_first": torch.ones(point_cloud_first.shape[0], dtype=torch.bool),
            "valid_mask_next": torch.ones(point_cloud_second.shape[0], dtype=torch.bool),
            "sequence_id": self.dataset_path,
            "k": k,
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            dict: A dictionary containing:
                ...
                - point_cloud_next (torch.Tensor): Next frame point cloud [N, 3]
