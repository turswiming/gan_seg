...
def initialize_models_and_optimizers(config, N, device):
    ...
    optimizer_mask = torch.optim.Adam(
        mask_predictor.parameters(), 
        lr=config.model.mask.lr,
...
def load_checkpoint(config, flow_predictor, mask_predictor, 
    ...
    if config.checkpoint.overwrite_flow_predictor:
        ...
        optimizer_flow=torch.optim.Adam(flow_predictor.parameters(), lr=config.model.flow.lr)#reload the optimizer to make it work
