...
skip_training = False
ab_name = "fix_loss_contribution"  # Name for the ablation study
...
for key in loss_contribution.keys():
    ...
    for i in range(run_times):
        ...
        command_list = [
            ...
            f"--config", "config/ablation_loss.yaml",
            f"log.dir={savepath}",
        ]
        if key == "without_dynamic":
            command_list.append("lr_multi.rec_loss=0")
        elif key == "without_rigid":
            command_list.append("losses.scene_flow_smoothness.each_mask_item.relative_gradient=0")
        elif key == "without_coverage":
            command_list.append("losses.scene_flow_smoothness.sum_mask_item.relative_gradient=0")
        elif key == "without_rigid_converage":
            command_list.append("lr_multi.scene_flow_smoothness=0")
        elif key == "baseline":
            pass
