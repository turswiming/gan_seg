    ...
    if config.lr_multi.KNN_loss > 0 and train_flow:
        ...
        for i in range(len(point_cloud_firsts)):
            if config.dataset.name == "KITTISF_new":
                ...
                point_cloud_firsts[i] = point_cloud_firsts[i][~ground_mask_first]
                point_cloud_nexts[i] = point_cloud_nexts[i][~ground_mask_next]
                pred_flow[i] = pred_flow[i][~ground_mask_first]
            pred_second_point = point_cloud_firsts[i][:, :3] + pred_flow[i]
            ...
            if cascade_flow_outs is not None:
                for cascade_flow in cascade_flow_outs:
                    pred_second_point_cascade = point_cloud_firsts[i][:, :3].to(device)+cascade_flow[i][~ground_mask_first]
